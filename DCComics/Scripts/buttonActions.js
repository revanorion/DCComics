$(document).ready(function () {
    ...
    $(document).on('click', '.ControlTypesTable tbody tr', function () {
        var row = table.row(this);

        var mydata = table.row(this).data();
        var headerText = $('.ControlTypesTable').data('modal-header');
        $.ajax({
            url: $('.ControlTypesTable').data('request-url'),
            type: 'GET',
            data: { 'id': mydata.ControlTypeId },
            dataType: 'html',
            success: function (data) {
                $('#modalHeaderText').html(headerText);
                $('#popUpEntityPartialView').html(data);
